...
import server.api_gateway.auth as auth
...
class FakeConnection:
    ...
    recv()/send()/close() plus async iteration for AuthGateway.authenticate to
...
def _run_authenticate(raw_messages, user_store=None):
    ...
    async def scenario():
        return await auth_gateway.authenticate(connection)
...
def test_decode_login_goes_through_the_shared_message_registry():
    ...
    # into exactly the (username, password, no rejection) tuple authenticate expects.
    username, password, reason = auth._decode_login(_login_message("alice"))

    assert (username, password, reason) == ("alice", "hunter2", None)


def test_valid_login_returns_the_username():
    connection, username, _ = _run_authenticate([_login_message("alice")])
...
def test_malformed_json_is_rejected_and_closed():
    connection, username, _ = _run_authenticate(["not json {"])
...
def test_wrong_message_type_is_rejected_and_closed():
    ...
    connection, username, _ = _run_authenticate([raw])
...
def test_a_connection_that_never_sends_anything_is_closed_after_the_timeout(monkeypatch):
    monkeypatch.setattr(auth, "LOGIN_TIMEOUT_S", 0.05)

    connection, username, _ = _run_authenticate([])
...
def test_a_new_username_creates_an_account():
    ...
    connection, username, _ = _run_authenticate([_login_message("alice", "hunter2")], user_store=user_store)
...
def test_an_existing_user_with_the_wrong_password_is_rejected_and_closed():
    ...
    connection, username, _ = _run_authenticate([_login_message("alice", "wrong-password")], user_store=user_store)
