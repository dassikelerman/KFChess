...
# -- allow_single_client_both_colors (dev flag, off by default) -------------


def test_dev_flag_off_by_default():
    session = Session(BOARD)
    assert session._allow_single_client_both_colors is False


def test_dev_flag_lets_the_white_connection_move_a_white_piece():
    session = Session(["wR . .", ". . ."], allow_single_client_both_colors=True)
    session.assign_role("conn-a")  # white
    network_publisher = FakeNetworkPublisher()
    session.network_publisher = network_publisher
...
def test_dev_flag_also_lets_the_white_connection_move_a_black_piece():
    session = Session(["wR . .", ". . bP", ". . ."], allow_single_client_both_colors=True)
    session.assign_role("conn-a")  # white
    network_publisher = FakeNetworkPublisher()
    session.network_publisher = network_publisher

    # bP at (1, 2) taking its single legal forward step to (2, 2) - black's
    # direction is +1 (constants.PAWN_DIRECTION).
    session.handle_client_message("conn-a", to_dict(MoveIntent(source=Position(1, 2), destination=Position(2, 2))))
    session.tick(constants.MOVE_DURATION + 1)
...
def test_dev_flag_does_not_extend_ownership_to_a_black_connection():
    session = Session(["wR . .", ". . ."], allow_single_client_both_colors=True)
    session.assign_role("conn-a")  # white
    session.assign_role("conn-b")  # black
    network_publisher = FakeNetworkPublisher()
