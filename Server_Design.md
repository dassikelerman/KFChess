...
**מצב היישום היום — reverse proxy מסיים-TLS מול backend יחיד, לא עדיין
Load Balancer.** שירות `tls_proxy` (Caddy) ב-`docker-compose.yml` מבצע
היום בדיוק תפקיד 1 בלבד למעלה: מסיים TLS אמיתי על `wss://localhost:8443`
(`deploy/Caddyfile`, `tls internal` — CA מקומי, לא ציבורי) ומעביר את
התעבורה ל-`ws_server` שעדיין רץ על המארח עצמו (`host.docker.internal:8765`)
— זה סוגר בפועל את פערי סיסמת ה-`Login` בטקסט גלוי. תפקיד 2 (**ניתוב בין
עותקי Gateway**) **עדיין לא קיים בפועל**: יש רק backend יחיד, כך שאין בין
מה לפזר — Load Balancing אמיתי יתחיל רק בשלב 6, כשל-Docker Compose יהיו
כמה עותקי Gateway לנתב ביניהם. עד אז `tls_proxy` הוא reverse proxy מסיים-
TLS מול יעד יחיד, לא Load Balancer במובן המלא. הלקוח (`client/
server_connection.py`) חייב הפעלה מפורשת של `KFCHESS_DEV_INSECURE_TLS=1`
כדי לבטוח ב-CA המקומי הזה — אימות מלא נשאר ברירת המחדל, ואינו רלוונטי
כלל מול פריסה אמיתית עם תעודה אמיתית. **מגבלה ידועה שנתגלתה תוך אימות
ידני:** על מכונה שבה `ssl.SSLContext` מוחלף גלובלית (למשל על ידי
`pip_system_certs`, דרך קובץ `.pth`), האימות מול OS trust store קורה בכל
מקרה, בלי קשר ל-`verify_mode`/`check_hostname` — מה שמנטרל את העוקף. זו
תכונת-מכונה חד-פעמית, לא באג בקוד; הפתרון שם הוא לבטוח ב-CA המקומי של
Caddy ישירות ב-OS trust store, לא שינוי קוד.
